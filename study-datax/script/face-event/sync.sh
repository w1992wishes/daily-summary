...
TIME_JSON_FILE=$TIME_JOSN".json"
TIME_TMP_FILE=$TIME_JOSN"_tmp.json"

SYNC_JOSN=`cat system.properties | grep $PROJECT_PREFIX".incrementSync" | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`
SYNC_JSON_FILE=$SYNC_JOSN".json"
SYNC_TMP_FILE=$SYNC_JOSN"_tmp.json"
...
python $DATAX_HOME/bin/datax.py $TIME_TMP_FILE
...
if [[ "$MAX_TIME" != "null" ]]; then
  ...
  # 将第 63 行的 truncate 语句删除；
  sed -i '63d' $SYNC_TMP_FILE
  # 增量更新
  python $DATAX_HOME/bin/datax.py $SYNC_TMP_FILE
  # 删除临时文件
  rm $SYNC_TMP_FILE
else
  # 全部更新
