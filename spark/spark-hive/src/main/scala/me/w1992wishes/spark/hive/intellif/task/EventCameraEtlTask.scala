...
import me.w1992wishes.spark.hive.intellif.param.EventCameraEtlCLParam
...
/**
  ...
  */
object EventCameraEtlTask {

  def main(args: Array[String]): Unit = {
    args.foreach(println(_))

    val clParam = new EventCameraEtlCLParam(args)
    ...
    if (!cameras.isEmpty) {
      ...
      var resultDF = sql("select * from eventCamera")
      if (isCoalesce) {
        resultDF = resultDF.coalesce(coalescePartitions)
      }
      resultDF
        .write
        .format("hive")
