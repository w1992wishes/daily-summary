...
/**
  * @author w1992wishes 2020/7/4 17:04
  */
object SameEntryExitAnalysisAgg extends DrugRelatedAnalysis with HiveAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 读取配置文件
    val sourceSql = ConfigArgsDrugRelated.hiveEntryExitAnalysisAggSourceSql
      .replace("#biz_code#", bizCode)
      .replace("#start_time#", outStartTime).replace("#start_dt#", startTime)
      .replace("#end_time#", outEndTime).replace("#end_dt#", endTime)
    val resultTable = ConfigArgsDrugRelated.hiveEntryExitAnalysisAggResultTable.replace("#biz_code#", bizCode)
