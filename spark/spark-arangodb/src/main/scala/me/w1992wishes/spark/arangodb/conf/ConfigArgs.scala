...
object ConfigArgs {
  ...
  private val config: Config = ConfigFactory.load("application.conf") // 父配置文件
  ...
  val gpAddress: String = config.getString("parent.db.gp.address")
  val gpDriver: String = config.getString("parent.db.gp.driver")
  val gpUser: String = config.getString("parent.db.gp.user")
  val gpPassword: String = config.getString("parent.db.gp.password")
  val gpFetchsize: String = config.getString("parent.db.gp.fetchsize")
