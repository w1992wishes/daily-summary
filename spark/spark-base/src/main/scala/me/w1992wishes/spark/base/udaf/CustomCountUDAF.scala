...
class CustomCountUDAF extends UserDefinedAggregateFunction {
  
  // inputSchema，指的是，输入数据的类型
  def inputSchema: StructType = {
    StructType(Array(StructField("input", LongType, nullable = true)))
  }
  ...
  // 由于Spark是分布式的，所以一个分组的数据，可能会在不同的节点上进行局部聚合，就是 update
  // 但是，最后一个分组，在各个节点上的聚合值，要进行 merge，也就是合并
