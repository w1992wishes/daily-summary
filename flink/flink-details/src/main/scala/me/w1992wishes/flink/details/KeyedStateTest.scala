...
/**
  * @author w1992wishes 2020/6/24 15:32
  */
object KeyedStateTest extends App {
  ...
  val sourceStream: DataStream[UserBehavior] = env
    .addSource(new UserBehaviorSource("UserBehavior.csv")).assignTimestampsAndWatermarks(new AscendingTimestampExtractor[UserBehavior]() {
...
class UserBehaviorSource(path: String) extends RichSourceFunction[UserBehavior] {
  ...
  override def run(sourceContext: SourceContext[UserBehavior]): Unit = {
    // 从项目的resources目录获取输入
    streamSource = KeyedStateTest.getClass.getClassLoader.getResourceAsStream(path)
